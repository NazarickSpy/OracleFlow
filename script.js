// =============================================
// ORAKELFLOW AI CHAT - SCRIPT.JS (FIXED VERSION)
// =============================================
...
(function() {
    ...
    // =============================================
    function init() {
        // Validasi elemen DOM yang penting
        if (!elements.chatContainer || !elements.promptInput || !elements.sendBtn) {
    ...
    function setupEventListeners() {
        ...
        if (!elements.sendBtn || !elements.promptInput) {
            console.error('Elemen penting untuk event listener tidak ditemukan');
            return;
        }
        ...
        // Sidebar
        if (elements.menuToggleBtn && elements.sidebarOverlay) {
    ...
    function updateSendButton() {
        if (!elements.sendBtn || !elements.promptInput) return;
    ...
    function toggleSidebar() {
        if (!elements.sidebarOverlay) return;
    ...
    function renderChatHistory() {
        ...
        state.allChats.forEach((chat, index) => {
            ...
            const firstUserMessage = chat.messages ? chat.messages.find(m => m.role === 'user') : null;
            const title = firstUserMessage ? 
                (firstUserMessage.content ? firstUserMessage.content.substring(0, 25) : 'Percakapan') + 
                (firstUserMessage.content && firstUserMessage.content.length > 25 ? '...' : '') : 
                'Percakapan Baru';
    ...
    function updateUI() {
        if (!elements.welcomeScreen || !elements.chatContainer) return;
    ...
    function renderMessages() {
        if (state.currentChatIndex === -1 || !state.allChats[state.currentChatIndex] || !elements.chatContainer) return;
    ...
    function handleFileUpload(e) {
        if (!e.target.files || !elements.filePreview) return;
    ...
    function toggleVoiceInput() {
        if (!state.recognition || !elements.voiceBtn) {
        ...
        if (state.isListening) {
            ...
            try {
                ...
                showToast('❌ Gagal memulai voice input', 'error');
            }
        }
    }

    // =============================================
    // MESSAGE SENDING & AI RESPONSE - FIXED
    // =============================================
    async function sendMessage() {
        ...
        // Pastikan messages array ada
        if (!state.allChats[state.currentChatIndex].messages) {
            state.allChats[state.currentChatIndex].messages = [];
        }
        ...
        try {
            ...
            if (error.message.includes('API key not valid')) {
                ...
                errorMessage = '⏰ Timeout: Respons terlalu lama. Silakan coba lagi.';
            }
